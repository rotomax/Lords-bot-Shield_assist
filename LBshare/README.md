...
recovers gracefully (so you can play on your phone without the bot
...
* **Windows 10 or 11**
* **Lords Mobile PC** installed (the official client, not an emulator)
* **Screen resolution: 1920×1080** (other resolutions will need
calibration — see below)
* **Python 3.10 or newer** (3.11 recommended)
* **Tesseract OCR** for Windows (separate installer)
...
Open `config.py` in any text editor (Notepad works). The two values you
most likely need to change are at the top of the **Game / Emulator**
section:
...
display to 1920×1080 before running the bot. Lords Mobile PC should be in
**windowed mode** at its default size — when you launch it the window will
sit roughly in the centre of your screen, and the bot auto-detects its
...
3. **Open Command Prompt** in the bot folder (Shift + right-click in the
...
**To stop the bot:** press `Ctrl+Shift+Q` from any window, or `Ctrl+C` in
...
The bot does NOT poll the screen every second — that would be wasteful and
...
device" dialog on the PC and the session goes dead. When the bot sees
this dialog it:

1. **Waits 30 minutes** before doing anything, so you can play on your
phone without the bot fighting you
2. **Watches your shield timer during the wait** — if the shield gets
close to expiring, the bot recovers immediately to protect it
...
used only to *detect* a purchase promo, so the bot knows to look for
the X close button instead.
* **Never clicks Confirm on the "Quit Game?" dialog.** Every ESC press
goes through `safe\_esc()`, which clicks Cancel if the quit dialog
appears.
* **Double-shield guard.** Before applying a shield, the bot re-reads
...
The bot is designed to handle this: after 5 consecutive failed shield
